...
process_execute (const char *file_name) 
{
  ...
  f_name = strtok_r((char*) f_name, " ", &save_ptr );
...
start_process (void *f_name)
{
  ...
  /*sets a custom status field in the current thread and then signals a semaphore */
  struct thread *curr_t = thread_current();
  curr_t->load_status = success;
  sema_up(&curr_t->start_sem);
...
int process_wait(tid_t child_tid) {
    ...
    sema_down(&child_t->prior_sem);

    int exit_status = child_t->exit_status;
    
    sema_up(&child_t->exit_sem);
...
process_exit (void)
{
  ...
  while(!list_empty(&cur->open_fd_list))
  {
    ...
    file_close(_file_descriptor->file);
    list_remove(&_file_descriptor->fd_elem);
    free(_file_descriptor);
  }
  
  sema_up(&cur->prior_sem);
  sema_down(&cur->exit_sem);
...
static bool setup_stack (void **esp, const char *f_name);
...
load (const char *f_name, void (**eip) (void), void **esp) 
{
  ...
  *eip = (void (*) (void)) ehdr.e_entry;
  ...
  {
    // block writing when process is running
    t->process_file = file;
...
/* Create a minimal stack by mapping a zeroed page at the top of
   user virtual memory. */
static bool
setup_stack (void **esp, const char *f_name) 
{
  uint8_t *kpage;
  bool success = false;
  ...
  if (kpage != NULL) 
  {
    ...
    if (success) {
      *esp = PHYS_BASE;
      ...
      f_name_ = palloc_get_page(0);
      if (f_name_ == NULL)
        return TID_ERROR;
      strlcpy(f_name_, f_name, PGSIZE);

      cmd_args_arr = malloc(DEFAULT_ARG_SIZE*sizeof(char*));
      for (
        token = strtok_r(f_name_, " ", &save_ptr); 
        token != NULL;
