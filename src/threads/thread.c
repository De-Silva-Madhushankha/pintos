...
thread_sleep_less(const struct list_elem *a, const struct list_elem *b,
                  void *aux UNUSED)
{
  const struct thread *thread_a = list_entry(a, struct thread, blockedelem);
  const struct thread *thread_b = list_entry(b, struct thread, blockedelem);
                    ...
                    thread_func *function, void *aux)
{
  ...
  sema_init(&t->prior_sem, 0);
  sema_init(&t->start_sem, 0);
